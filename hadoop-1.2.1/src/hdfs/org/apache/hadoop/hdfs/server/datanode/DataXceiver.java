...
class DataXceiver implements Runnable, FSConstants {
  ...
  public void run() {
    ...
    try {
      ...
      LOG.info("Received version=" + version);
      ...
      case DataTransferProtocol.OP_WRITE_BLOCK:
        ...
        break;
      case DataTransferProtocol.OP_TOUCH_BLOCK:
        touchBlock( in );
        break;
  ...
  private void readBlock(DataInputStream in) throws IOException {
    ...
    LOG.info("In readBlock, " + length + "," + block.getNumBytes());
  ...
  private void touchBlock(DataInputStream in) throws IOException {
    LOG.info("In touchBlock");
    Block block = new Block(in.readLong(), 
        dataXceiverServer.estimateBlockSize, in.readLong());
    String localFileName = Text.readString(in);
    LOG.info("localFileName is " + localFileName);
    DataOutputStream out = new DataOutputStream(NetUtils.getOutputStream(s,
        datanode.socketWriteTimeout));
    out.writeLong(datanode.data.touchToBlock(block, localFileName, 0));
    datanode.notifyNamenodeReceivedBlock(block, DataNode.EMPTY_DEL_HINT);
    //datanode.
    try {
      out.writeShort(DataTransferProtocol.OP_STATUS_SUCCESS);
      Text.writeString(out, "Zhe-line");
      out.flush();
    } finally {
      IOUtils.closeStream(out);
    }
    ...
    try {
      ...
      Path localFileDirPath = localFilePath.getParent();
      DataNode.LOG.info("Adding watch on " + localFileDirPath.toString());
      WatchKey key = localFileDirPath.register(watcher,
       ENTRY_MODIFY);
      LocalFileWatcher lfw = new LocalFileWatcher(watcher, localFilePath, block);
      lfw.start();
    } catch (IOException x) {
      DataNode.LOG.info(x);
    }
    ...
    private class LocalFileWatcher extends Daemon {
    private WatchService watcher;
    private Path pathToWatch;
    Block block;
    private long offsetInFile;
    public LocalFileWatcher(WatchService w, Path p, Block b) {
      watcher = w;
      pathToWatch = p;
      block = b;
      offsetInFile = 0;
    ...
    public void run() {
      DataNode.LOG.info("Watcher added for " + pathToWatch.toString());
      for (;;) {
        WatchKey watchKey;
        try{
         ...
         for (WatchEvent<?> event: watchKey.pollEvents()){
          ...
          if (changed.endsWith(pathToWatch.getFileName().toString())) {
            long newFileLen = pathToWatch.toFile().length();
            if (newFileLen == datanode.data.getVisibleLength(block))
              continue;
            DataNode.LOG.info("Local file changed " + changed + 
              ", new length is " + newFileLen);
            //TOFIX: programs like vim shrink the file and grow it later; this only works for appending
            long updatedOldBlkLen = (newFileLen - offsetInFile <=  DEFAULT_BLOCK_SIZE) ?
                                      newFileLen - offsetInFile : DEFAULT_BLOCK_SIZE;

            block.setNumBytes(updatedOldBlkLen);
            // might need to add an update function in FSDataset too
            datanode.data.setVisibleLength(block, updatedOldBlkLen);
            datanode.notifyNamenodeUpdatedBlock(block, DataNode.EMPTY_DEL_HINT);

            if (newFileLen - offsetInFile > DEFAULT_BLOCK_SIZE) {
              DataNode.LOG.info("Need to notify namenode for new block");
              
              Block newBlock = new Block();
              offsetInFile += DEFAULT_BLOCK_SIZE;
              Block retBlk = datanode.namenode.newBlockForLocalFile(datanode.dnRegistration, 
                                newBlock);
              newBlock.setBlockId(retBlk.getBlockId());
              newBlock.setGenerationStamp(retBlk.getGenerationStamp());
              DataNode.LOG.info("newBlockForLocalFile returned " + retBlk.getBlockId());
              datanode.data.touchToBlock(newBlock, pathToWatch.toString(), offsetInFile);
              DataNode.LOG.info("New offsetInFile " + offsetInFile + ", newBlock size is " + newBlock.getNumBytes());

              //newBlock.setNumBytes(newFileLen - offsetInFile - DEFAULT_BLOCK_SIZE);
              datanode.notifyNamenodeReceivedBlock(newBlock, DataNode.EMPTY_DEL_HINT);
              this.block = newBlock;
            }
          }
        }
  ...
  private void copyBlock(DataInputStream in) throws IOException {
    ...
    try {
      ...
      blockSender = new BlockSender(block, 0, -1, true, false, false, 
          datanode);
