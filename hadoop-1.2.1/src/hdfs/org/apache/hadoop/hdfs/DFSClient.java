...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  public OutputStream create(String src, 
                            ArrayList<String> composeConfig,
                             FsPermission permission,
                             boolean overwrite, 
                             boolean createParent,
                             short replication,
    ...
    System.out.println("DFSClient, creating OutputStream");
    checkOpen();
    if (permission == null) {
      permission = FsPermission.getDefault();
    }
    FsPermission masked = permission.applyUMask(FsPermission.getUMask(conf));
    LOG.debug(src + ": masked=" + masked);
    final DFSOutputStream result = new DFSOutputStream(src, composeConfig, 
        masked,
        overwrite, createParent, replication, blockSize, progress, buffersize,
        conf.getInt("io.bytes.per.checksum", 512));
    beginFileLease(src, result);
  ...
  public static class RemoteBlockReader extends FSInputChecker implements BlockReader {
    ...
    public static BlockReader newBlockReader( Socket sock, String file,
      ...
      //write the header.
      LOG.info("Creating newBlockReader, " + blockId + "," + genStamp + "," + len);
      //genStamp = 1001;
      out.writeShort( DataTransferProtocol.DATA_TRANSFER_VERSION );
  ...
  public class DFSInputStream extends FSInputStream {
    ...
    DFSInputStream(String src, int buffersize, boolean verifyChecksum
      ...
      LOG.info("Creating DFSInputStream");
    ...
    private boolean updateBlockInfo(LocatedBlocks newInfo) throws IOException {
      LOG.info("In updateBlockInfo," + serverSupportsHdfs200 + "," + newInfo.isUnderConstruction()
                + "," + newInfo.locatedBlockCount());
      if (!serverSupportsHdfs200 || !newInfo.isUnderConstruction()
          || !(newInfo.locatedBlockCount() > 0) || newInfo.isUnderConstruction()) {
        // ZHE-TOFIX
        return true;
      }
      ...
      for (int i = 0; i < last.getLocations().length && newBlock == null; i++) {
        ...
        try {
          LOG.info("In updateBlockInfo, dn = " + datanode.getName());
  ...
  ****************************************************************/
  class DFSOutputStream extends FSOutputSummer implements Syncable {
     ...
     * @see ClientProtocol#create(String, FsPermission, String, boolean, short, long)
     */
    DFSOutputStream(String src, ArrayList<String> composeConfig, FsPermission masked, 
        boolean overwrite,
        boolean createParent, short replication, long blockSize, Progressable progress,
        int buffersize, int bytesPerChecksum) throws IOException {
      this(src, blockSize, progress, bytesPerChecksum, replication);
      ...
      //if (src.contains("zz")) {
        //byte[] b = new byte[1]();
      for (String s : composeConfig) {
        LOG.info("In creating DFSOutputStream, nextBlockOutputStream " + s);
        nodes = nextBlockOutputStream(true, s);
        LOG.info("Returned from nextBlockOutputStream " + s);
      }
        //writeChunk();
      if (nodes != null && nodes.length > 0) {
        LOG.info("Returned " + nodes.length + " nodes");
        LOG.info("Selected node: " + nodes[0].getName());
      }
      //}
    ...
    private DatanodeInfo[] nextBlockOutputStream() throws IOException {
      return nextBlockOutputStream(false, null);
    }
    /**
     ...
     */
    private DatanodeInfo[] nextBlockOutputStream(boolean touchBlock, String composeConfig) 
        throws IOException {
      ...
      do {
        ...
        String composeLocalFile = null;

        if (touchBlock) {
          String composeHost = NetUtils.normalizeHostName(composeConfig.split(",")[0]);
          composeLocalFile = composeConfig.split(",")[1];
          String composeDataNodeName = composeHost + ":50010";
          LOG.info("Setting chosenNode to " + composeDataNodeName);
          DatanodeInfo chosenNode = new DatanodeInfo(new DatanodeID(composeDataNodeName));

          //lb = locateFollowingBlock(startTime, excluded.length > 0 ? excluded : null);
          try {
            if (serverSupportsHdfs630) {
              lb = namenode.addTouchedBlock(src, clientName, excluded, chosenNode);
            } else {
              lb = namenode.addTouchedBlock(src, clientName, chosenNode);
            }
          } catch (Exception e) {
            LOG.info("Unable to add block at NameNode.");
            throw new IOException("Unable to add block at NameNode.");
          }
        } else {
          lb = locateFollowingBlock(startTime, excluded.length > 0 ? excluded : null);
        }
        ...
        if (nodes != null && nodes.length > 0) {
          LOG.info("Selected node: " + nodes[0].getName() + "," + block.getNumBytes() + " bytes");
        }
        ...
        success = createBlockOutputStream(nodes, clientName, false, 
          touchBlock, composeLocalFile, lb);
      ...
      LOG.info("Returning from nextBlockOutputStream");
      return nodes;
    }

private boolean createBlockOutputStream(DatanodeInfo[] nodes, String client,
                    boolean recoveryFlag) {
  return createBlockOutputStream(nodes, client, recoveryFlag, false, null, null);
}
    // connects to the first datanode in the pipeline
    // Returns true if success, otherwise return failure.
    //
    private boolean createBlockOutputStream(DatanodeInfo[] nodes, String client,
                    boolean recoveryFlag, boolean touchBlock, 
                    String localFileName, LocatedBlock lb) {
      ...
      try {
        ...
        out.writeShort( DataTransferProtocol.DATA_TRANSFER_VERSION );
        if (touchBlock == true) {
          LOG.info("Sending touch block");
          out.write( DataTransferProtocol.OP_TOUCH_BLOCK );
          LOG.info("After sending touch block");
        } else {
        ...
        out.writeLong( block.getGenerationStamp() );
        if (touchBlock) {
          Text.writeString(out, localFileName);
        }
        ...
        for (int i = 1; i < nodes.length; i++) {
          LOG.info("Doing write for node " + i);
          nodes[i].write(out);
        }
        accessToken.write(out);
        checksum.writeHeader( out );
        LOG.info("Before doing flush");
        out.flush();
        LOG.info("After doing flush");

        // receive ack for connect
        if (touchBlock) {
          long fileLength = blockReplyStream.readLong();
          LOG.info("Local file length is " + fileLength);
          //block.setNumBytes(fileLength);
        }
        pipelineStatus = blockReplyStream.readShort();
        firstBadLink = Text.readString(blockReplyStream);
        LOG.info("Block reply stream status is " + pipelineStatus);
